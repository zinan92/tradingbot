#!/usr/bin/env python3
"""
Custom download script with user-specified symbols
"""
...
async def start_custom_download():
    """Start download with custom symbol list"""
    ...
    print("=" * 80)
    print("CUSTOM BINANCE FUTURES HISTORICAL DATA DOWNLOAD")
    print("=" * 80)
    
    # Check what we already have
    already_complete = ["ETHUSDT", "BTCUSDT", "SOLUSDT"]
    partial_data = ["BNBUSDT"]
    new_symbols = ["AAVEUSDT", "JUPUSDT", "1000BONKUSDT", "JTOUSDT", "UNIUSDT", 
                   "SUSHIUSDT", "CRVUSDT", "LDOUSDT", "PENDLEUSDT", "ONDOUSDT", "1000SHIBUSDT"]
    
    # Filter symbols to download
    symbols_to_download = [s for s in CUSTOM_SYMBOLS if s not in already_complete]
    
    print(f"\n📊 Symbol Summary:")
    print(f"   Total symbols: {len(CUSTOM_SYMBOLS)}")
    print(f"   Already complete: {len(already_complete)} ({', '.join(already_complete)})")
    print(f"   To download: {len(symbols_to_download)}")
    print(f"   New symbols: {len(new_symbols)}")
    
    print(f"\n⚠️  This will download 3 YEARS of data for {len(symbols_to_download)} symbols")
    print(f"   Estimated time: {len(symbols_to_download) * 0.3:.1f} - {len(symbols_to_download) * 0.5:.1f} hours")
    print(f"   Estimated storage: {len(symbols_to_download) * 0.15:.1f} - {len(symbols_to_download) * 0.2:.1f} GB")
    print(f"   Estimated candles: ~{len(symbols_to_download) * 520000:,}")
    ...
    try:
        ...
        for i, symbol in enumerate(symbols_to_download, 1):
            marker = " 🆕" if symbol in new_symbols else ""
            print(f"   {i:2d}. {symbol}{marker}")
        ...
        validation = data_manager.validate_data_integrity(
            symbols=CUSTOM_SYMBOLS,  # Validate all symbols including already complete
            intervals=intervals,
            start_date=start_date,
            end_date=end_date
        )
        ...
        # Save report
        report_file = f"custom_download_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        import json
        with open(report_file, 'w') as f:
            json.dump({
                'symbols': CUSTOM_SYMBOLS,
                'downloaded': symbols_to_download,
        ...
        print("\n✨ Custom download completed successfully!")
        return True
...
if __name__ == "__main__":
    success = asyncio.run(start_custom_download())
